...
const handleAttendance = async (req, res) => {
    ...
    try {
        ...
        if (specialCamera?.subjectId) {
            activeSubject = await Subject.findById(specialCamera.subjectId).populate("doctor teachingAssistant");
            activeSchedule = specialCamera.groupNumber
        ...
        if (!activeSubject || !activeSchedule) {
            ...
            for (const subject of subjects) {
                ...
                for (const schedule of schedules) {
                    if (
                        schedule.schedule &&
                        ((type === "ROOM" && schedule.schedule.roomNumber === parseInt(number)) ||
                            (type === "LAB" && schedule.schedule.labNumber === parseInt(number))) &&
                        now.getDay() === ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"].indexOf(schedule.schedule.day) &&
                        now.getHours() + now.getMinutes() / 60 >= parseTime(schedule.schedule.startTime) &&
                        now.getHours() + now.getMinutes() / 60 <= parseTime(schedule.schedule.endTime)
                    ) {
        ...
        if (!activeSubject || !activeSchedule) {
            return res.status(400).send(`Fail|No Active ${type === "ROOM" ? "Lec" : "Sec"}`);
        }
        ...
        const currentWeek = Math.floor((now - startWeekDate) / (7 * 24 * 60 * 60 * 1000)) + 1;
        ...
        const attendanceCount = await Attendance.countDocuments({
            subject: activeSubject._id,
            week: currentWeek,
            ...(activeSchedule.groupNumber ? { group: activeSchedule.groupNumber, lectureAttendanceTime: { $ne: null } } : {}),
            ...(activeSchedule.sectionNumber ? { section: activeSchedule.sectionNumber, sectionAttendanceTime: { $ne: null } } : {}),
        });

        const role = activeSchedule.sectionNumber ? "TA" : "Dr";
        const name = activeSchedule.sectionNumber
            ? activeSubject.teachingAssistant?.name || "Unknown"
            : activeSubject.doctor?.name || "Unknown";
...
const getCurrentSubject = async (req, res) => {
...
const getCamerasList = async (req, res) => {
...
const updateCameraInfo = async (req, res) => {
    const { cameraId, subjectId, groupNumber, sectionNumber } = req.body;
    const update = {};
